...
var CellularAutomatonEngine = /** @class */ (function () {
  ...
  CellularAutomatonEngine.randBoard = function (size, density) {
    ...
    return new Uint8Array(size * size).map(function () {
      return _this.rand(0, 2, density);
    });
  };
...
window.setInterval(function () {
  return (favicon.href = canvas.toDataURL());
}, 5000);
var msPast = null;
var msPerFrame = 7;
...
setTimeout(function () {
  return log("Hover over controls for help");
}, 3000);
...
sel("#delay").addEventListener(
  "input",
  function (e) {
    return (simulation.alpha = rangeOver(e.target.value, 1.0, 0));
  },
  false
);
var blendModeLink = {
...
sel("#rate").addEventListener("input", function (e) {
  return (msPerFrame = parseInt(e.target.value));
});
...
canvas.addEventListener(
  "mousemove",
  function (e) {
    return isHovering && simulation.hover(e);
  },
  false
);
canvas.addEventListener(
...
sel("#modal-capture-preview").addEventListener(
  "click",
  function () {
    return (sel("#modal-capture ").style.display = "none");
  },
  false
);
...
sel("#showGallery").addEventListener("click", function () {
  return (sel("#modal-capture").style.display = "flex");
});
...
sel("#seedDensity").addEventListener("input", function (e) {
  return (simulation.seedDensity = parseInt(e.target.value));
});
...
sel("#colorRadix").addEventListener("input", function (e) {
  return (simulation.colorRadix = parseInt(e.target.value));
});
var recorders = null;
sel("#recStart").addEventListener("change", function () {
  ...
  recorders = rec;
  rec.ondataavailable = function (chunk) {
    return chunks.push(chunk.data);
  };
...
sel("#noiseRangeValue").addEventListener("input", function (e) {
  return (simulation.noiseRangeValue = rangeOver(e.target.value, 3, 12));
});
sel("#noiseRangeValue").addEventListener("change", function () {
  if (simulation.noiseEnabled) {
    log(
      "Noise (ON) Amount is now 1 in " +
        simulation.noiseRangeValue.toFixed(2) +
        " chance of being born"
    );
  } else {
    log(
      "Noise (OFF) Amount is now 1 in " +
        simulation.noiseRangeValue.toFixed(2) +
        " chace of being born"
    );
  }
...
setInterval(function () {
  var sum = simulation.data.reduce(function (a, b) {
    return a + b;
  }, 0);
