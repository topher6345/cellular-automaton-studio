...
class CellularAutomatonEngine {
    constructor(size, canvas, controlValues) {
        this.size = size;
        this.pixelSize = 1;
        this.pixelScalar = 1;
    ...
    static randBoard(size, density = 0) {
    ...
    update() {
        for (let i = 0; i < this.buffer.length; i++) {
            // Noise
            const spontaneousBirth = this.noiseEnabled &&
            ...
            switch (this.game) {
                case "famine":
                    ( // S6789
                    (alive && (liveNeighbors > 5)) || spontaneousBirth) && (status = 1);
                    break;
                case "anneal":
                    ( // S35678
                    (alive && (liveNeighbors === 3 || liveNeighbors === 5 || liveNeighbors === 6 || liveNeighbors === 7 || liveNeighbors === 8)) ||
                        // B4678
                        (liveNeighbors === 4 || liveNeighbors === 6 || liveNeighbors === 7 || liveNeighbors === 8)) && (status = 1);
                    break;
                case "morley":
                    ( // S245
                    (alive && (liveNeighbors === 2 || liveNeighbors === 4 || liveNeighbors === 5)) ||
                        // B368
                        (liveNeighbors === 3 || liveNeighbors === 6 || liveNeighbors === 8)) && (status = 1);
                    break;
                case "day&night":
                    ( // S34678
                    (alive && (liveNeighbors === 3 || liveNeighbors === 4 || liveNeighbors === 6 || liveNeighbors === 7 || liveNeighbors === 8)) ||
                        // B3678
                        (liveNeighbors === 3 || liveNeighbors === 6 || liveNeighbors === 7 || liveNeighbors === 8)) && (status = 1);
                    break;
                case "2x2":
                    ( // S125
                    (alive && (liveNeighbors === 1 || liveNeighbors === 2 || liveNeighbors === 5)) ||
                        // B36
                        (liveNeighbors === 3 || liveNeighbors === 6)) && (status = 1);
                    break;
                case "diamoeba":
                    ( // S5678
                    (alive && (liveNeighbors === 5 || liveNeighbors === 6 || liveNeighbors === 7 || liveNeighbors === 8)) ||
                        // B35678
                        (liveNeighbors === 3 || liveNeighbors === 5 || liveNeighbors === 6 || liveNeighbors === 7 || liveNeighbors === 8)) && (status = 1);
                    break;
                case "34life":
                    ( // S34
                    (alive && (liveNeighbors === 3 || liveNeighbors === 4)) ||
                        ...
                        spontaneousBirth) && (status = 1);
                    break;
                case "B25/S4":
                    ( // S4
                    (alive && (liveNeighbors === 4)) ||
                        // B25
                        (liveNeighbors === 2 || liveNeighbors === 5)) && (status = 1);
                    break;
                case "seeds":
                    ( // S
                    (alive) ||
                        // B2
                        (liveNeighbors === 2)) && (status = 1);
                    break;
                case "replicator":
                    ( // S1357
                    (alive && (liveNeighbors === 1 || liveNeighbors === 3 || liveNeighbors === 5 || liveNeighbors === 7)) ||
                        // B1357
                        (liveNeighbors === 1 || liveNeighbors === 3 || liveNeighbors === 5 || liveNeighbors === 7)) && (status = 1);
                    break;
                case "gems":
                    ( // S4568
                    (alive && (liveNeighbors === 4 || liveNeighbors === 5 || liveNeighbors === 6 || liveNeighbors === 8)) ||
                        // B3457
                        (liveNeighbors === 3 || liveNeighbors === 4 || liveNeighbors === 5 || liveNeighbors === 7)) && (status = 1);
                    break;
                case "fredkin":
                    ( // S1357
...
const simulation = new CellularAutomatonEngine(750, canvas, INIT_CONTROL_VALUES);
const favicon = sel("#favicon");
// Update the favicon with the current canvas
favicon.href = canvas.toDataURL();
window.setInterval(() => (favicon.href = canvas.toDataURL()), 5000);
let msPast = null;
let msPerFrame = 7;
let masterOnOff = true;
function tick(now) {
    if (!msPast)
        msPast = now;
...
window.requestAnimationFrame(tick);
const log = (message, link, linkText) => {
...
const rangeOver = (input, max, floor) => expon(input) * max + floor;
const expon = (x) => {
...
sel("#delay").addEventListener("input", (e) => {
    debugger;
    simulation.alpha = rangeOver(e.target.value, 0.1, 0.00001);
...
sel("#rate").addEventListener("input", (e) => (msPerFrame = parseInt(e.target.value)));
sel("#rate").addEventListener("change", () => log(`Speed is now ${msPerFrame} milliseconds per generation`));
...
canvas.addEventListener("mousemove", (e) => isHovering && simulation.hover(e), false);
...
sel("#modal-capture-preview").addEventListener("click", () => (sel("#modal-capture ").style.display = "none"), false);
const takeSnapshot = () => {
    const img = new Image();
...
sel("#showGallery").addEventListener("click", () => (sel("#modal-capture").style.display = "flex"));
...
sel("#seedDensity").addEventListener("input", (e) => (simulation.seedDensity = parseInt(e.target.value)));
sel("#seedDensity").addEventListener("change", (e) => log(`Seed Density changed to ${e.target.value}`));
const shapeLink = (shape) => {
    switch (shape) {
        case "gliderse":
        case "glidersw":
        case "gliderne":
...
// HSLUV picker
sel(".input-hex").addEventListener("input", (e) => {
    simulation.color = e.target.value;
    // redraw if paused so the user can see what colors
    masterOnOff || simulation.draw(false);
}, false);
...
sel("#colorRadix").addEventListener("input", (e) => (simulation.colorRadix = parseInt(e.target.value)));
let recorders = null;
...
sel("#noiseRangeValue").addEventListener("input", (e) => (simulation.noiseRangeValue = rangeOver(e.target.value, 3, 12)));
